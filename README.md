...
For screen alignment, a finite `window.orientation` takes precedence over `screen.orientation.angle`: Safari's legacy angle uses the portrait-relative motion frame, while the Screen Orientation API can use a different natural screen orientation. The modern API remains a fallback where the legacy value is unavailable. Regression tests cover conflicting orientation values, including a landscape tablet whose modern angle is zero.
