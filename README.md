...
The LEDs are thicker curved rectangles with gently rounded corners: both long edges follow the bowl circumference, and the segments sit closer to the inner edge. The nearest segment to the downhill direction uses a power curve (tilt magnitude to the power 0.45), so slight tilts are clearly visible. Above 75% tilt, an additional warm core and broad orange glow fade in smoothly, reaching full strength at maximum tilt. All lights are off at neutral. This only changes feedback: sensor sensitivity and fluid forces stay the same.
