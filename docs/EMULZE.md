...
- **Návrat tvarů začíná dříve.** Místní rozpustitelnost klesá s časovou konstantou 7 sekund místo 28. Drobný třes má toleranční pásmo: obnovu nezpomaluje ani znovu nerozpouští barvy. Teprve výraznější kroužení plynule tlumí oddělování a spojování oblastí. Nečekáme na dokonale nehybný iPad.
- **Spojování působí i na širší okolí, rozpouštění jen na přímé sousedy.** Širší spojování nastupuje místně až s poklesem rozpustitelnosti, aby po zastavení nezahladilo ještě promíchaná vlákna. Drobné kapky pak sdružuje do větších celků; u vytvořených hran se tlumí. Paměť promíchání i její sedmisekundový útlum zůstávají stejné. Jde o výměnu koncentrace, nikoli rozmazání výsledného obrazu.
...
Pro porovnání: `?drift=0` vypne jemné proudění, `?organic=0` jen chemickou protiváhu. Obě volby společně vypnou novější tvarové vlivy; aktuální rychlost návratu fází zůstane. `?dissolve=0` zachová míchání bez rozpouštění, `?stir=0` vypne přidaný míchací proud. Verze před zrychlením návratu je `a0f99d8`, před jemným prouděním `38d81ec`. Změny lze vracet samostatnými Git reverty.
